"use client";
import { useState, useEffect } from "react";
import { product } from "@/data/product";

const WA = process.env.NEXT_PUBLIC_WHATSAPP ?? "212600000000";

export default function StickyBar() {
  const [visible, setVisible] = useState(false);

  useEffect(() => {
    const fn = () => setVisible(window.scrollY > 500);
    window.addEventListener("scroll", fn, { passive: true });
    return () => window.removeEventListener("scroll", fn);
  }, []);

  const openOrder = () => {
    window.dispatchEvent(new CustomEvent("open-order", {
      detail: { ...product, image: product.images.hero },
    }));
  };

  if (!visible) return null;

  return (
    <div className="fixed bottom-0 inset-x-0 z-50 sm:hidden pb-safe">
      <div className="bg-white border-t border-gray-100 shadow-[0_-4px_20px_rgba(0,0,0,0.1)] px-4 py-3 flex gap-2 items-center">
        <div className="flex-1 min-w-0">
          <p className="font-black text-gray-900 text-sm truncate">{product.nom}</p>
          <p className="text-orange-500 font-bold text-base">{product.prix}</p>
        </div>
        <button
          onClick={openOrder}
          className="bg-orange-500 text-white font-black px-5 py-3.5 rounded-xl text-sm shrink-0"
        >
          Commander
        </button>
        <a
          href={`https://wa.me/${WA}?text=${encodeURIComponent("Bonjour, je veux commander : " + product.nom)}`}
          target="_blank" rel="noopener noreferrer"
          className="w-12 h-12 flex items-center justify-center bg-[#25D366] rounded-xl shrink-0"
        >
          <svg className="w-5 h-5 text-white" viewBox="0 0 24 24" fill="currentColor">
            <path d="M17.472 14.382c-.297-.149-1.758-.867-2.03-.967-.273-.099-.471-.148-.67.15-.197.297-.767.966-.94 1.164-.173.199-.347.223-.644.075-.297-.15-1.255-.463-2.39-1.475-.883-.788-1.48-1.761-1.653-2.059-.173-.297-.018-.458.13-.606.134-.133.298-.347.446-.52.149-.174.198-.298.298-.497.099-.198.05-.371-.025-.52-.075-.149-.669-1.612-.916-2.207-.242-.579-.487-.5-.669-.51-.173-.008-.371-.01-.57-.01-.198 0-.52.074-.792.372-.272.297-1.04 1.016-1.04 2.479 0 1.462 1.065 2.875 1.213 3.074.149.198 2.096 3.2 5.077 4.487.709.306 1.262.489 1.694.625.712.227 1.36.195 1.871.118.571-.085 1.758-.719 2.006-1.413.248-.694.248-1.289.173-1.413-.074-.124-.272-.198-.57-.347z"/>
            <path d="M12 0C5.373 0 0 5.373 0 12c0 2.13.558 4.13 1.533 5.863L.063 23.5l5.773-1.514A11.935 11.935 0 0012 24c6.627 0 12-5.373 12-12S18.627 0 12 0zm0 22c-1.99 0-3.85-.54-5.45-1.48l-.39-.23-4.028 1.057 1.074-3.92-.25-.4A9.953 9.953 0 012 12C2 6.477 6.477 2 12 2s10 4.477 10 10-4.477 10-10 10z"/>
          </svg>
        </a>
      </div>
    </div>
  );
}
